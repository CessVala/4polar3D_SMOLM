%% remove error print NB_modifs 09072010
...
byte_order = char(fread(TIF.file, 2, 'uchar')); %% compat octave
...
%% gestion waitbar nb_modif 13/12/2010
cptim = 0 ;
H = waitbar(0) ; 

while (ifd_pos ~= 0)
  
   %waitbar(cptim/(img_last-img_first), H, ''); cptim = cptim+1 ; %% gestion waitbar nb_modif 13/12/2010
   waitbar(cptim/(img_last), H, ''); cptim = cptim+1 ; %% gestion waitbar nb_modif 19/07/2011
...
bytes = fread( TIF.file, StripLength, classname, 0, TIF.BOS );   %% compat octave
...
return;
end%function %% nb_modif 09072010
...
global TIF;
entry.tiffType = fread(TIF.file, 1, 'uint16',0, TIF.BOS);   %% compat octave
...
if TIF.entry_tag == 33629   %special metamorph 'rationals'
    entry.val = fread(TIF.file, 6*entry.cnt, entry.matlabType,0, TIF.BOS);   %% compat octave
else
    if entry.tiffType == 5
        entry.val = fread(TIF.file, 2*entry.cnt, entry.matlabType,0, TIF.BOS);   %% compat octave
    else
        entry.val = fread(TIF.file, entry.cnt, entry.matlabType,0, TIF.BOS);   %% compat octave
    end
end
