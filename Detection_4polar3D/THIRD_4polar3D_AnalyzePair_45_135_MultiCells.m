% =========================================================================
% Script Name: THIRD_4polar3D_AnalyzePair_45_135_MultiCells.m
%
% Description:
%   This script performs the analysis of the second polarized projection pair: 45° and 135°.
...
%     - Requires the user to draw a single ROI around the bottom right (45°) polarized projection.
%     - The user must drag the same ROI across to the bottom left (135°) polarized projection 
%       to ensure the selected regions have exactly the same number of pixels.
...
%   3. Draw a single ROI around the bottom right (45°) polarized projection.
%   4. Drag this same ROI across to the bottom left (135°) polarized projection to guarantee 
%      identical pixel coverage.
%   5. The script will automatically apply the alignment and 4polar3D analysis to each selected stack.
%
% Notes:
%   - This script should typically be run **before** or **in parallel** with 
%     SECOND_4polar3D_AnalyzePair_0_90_MultiCells.m.
%   - Both scripts can be run **in parallel in two separate MATLAB windows** to process the
%     corresponding polarized pairs independently.
...
for k_index = 1:num_selected
    ...
    %% Run analysis for the 45° and 135° projections
    try
        tic;
        [tab_traj, tab_traj_all] = STHERMver2([NumStart, NumImgVector], [NumStart, NumImg], ...
            ...
            [FileName(1:end-4) '_' num2str(pfa) '_r_' num2str(r0) '_W_' num2str(wn) '_incl45New.mat']);
        toc;
    catch exception
